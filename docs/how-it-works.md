...
least glamorous part and the most reliably useful, because in real organisations
...
can actually answer confidently — then roll those judgements up to whatever
level the business cares about. The recording goes all the way to the bottom
...
and you can pull the model-generated judgements out of any measurement that
...
pull the sample, show each item next to its source, capture the judgement, write
...
Being honest about the limit: that holds for tools that go through the gate.
Anything else with write access — a command line, a database client, a
...
every organisation that has tried it.
